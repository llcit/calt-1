...
def show_questionnaire(request, runinfo, errors={}):
    ...
    request.runinfo = runinfo
    media=MediaUrl.objects.filter(url=request.path, user=request.user.id).count()
...
def generate_run(request, questionnaire_id, subject_id=None):
    ...
    if subject_id is not None:
        su = get_object_or_404(Subject, account=user.id)
    else:
        su = Subject.objects.filter(account=user.id, surname='Test')[0:1]
        if su:
            su = su[0]
        else:
            su = Subject(givenname=user.username, surname='Test', account=user)
            su.save()
...
def ajaxrequest(request):
    if request.is_ajax() and request.method == 'POST':
        user=request.user
        url=request.POST.get('suburl')
        audio_file=request.POST.get('audio')
        insert=MediaUrl(user=user, url=url, name_file=audio_file)
        insert.save()
